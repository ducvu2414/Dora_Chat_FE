...
export default function AudioCallComponent() {
  ...
  const conversation =
    conversations.find((c) => c._id === currentCall?.conversationId) || {};
  ...
  useEffect(() => {
    const startCall = async () => {
      ...
      try {
        ...
        await track
          .applyConstraints(getOptimalAudioConstraints())
          .catch((e) =>
            console.warn("Could not apply additional constraints:", e)
          );
        ...
        try {
          ...
          console.warn(
            "Advanced audio processing not available, using standard stream",
        ...
        AlertMessage({
          type: "error",
          message:
            "Unable to access microphone\nPlease check your browser's microphone access permissions",
        });
        handleEndCall();
      }
    };
    ...
    const timeout = setTimeout(() => {
      if (isConnecting) {
        console.warn(
          "⏳ Timeout: Không kết nối được sau 30s, kết thúc cuộc gọi."
        );
        handleEndCall();
      }
    }, 30 * 1000);
  ...
  const handleEndCall = async () => {
    ...
    if (
      ...
      typeof window.audioContextInstance.close === "function"
    ) {
                  ...
                  {partner[0].avatar ? (
                    ...
                    <div className="w-full h-full bg-blue-600 flex items-center justify-center">
                      <span className="text-white text-2xl">
                        {partner[0].name?.charAt(0)}
                      </span>
                    </div>
                  )}
                </div>
                <p className="text-xl font-semibold">
                  {partner[0].name || "Người dùng"}
                </p>
              </div>
            </div>
          );
